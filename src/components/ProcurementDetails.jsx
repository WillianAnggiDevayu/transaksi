...
export function PackagingTable({ lines = [], variant = "default" }) {
  ...
  const first = lines[0];
  const pagination = usePagination(lines, first?.detail_supplier_quotation_id || first?.detail_purchase_order_id || first?.clientId);
  return <div className={`border border-slate-200 ${supplier ? "rounded-xl" : "rounded-lg"}`}><div className="overflow-x-auto"><table className={`w-full min-w-[850px] text-left ${supplier ? "text-[13px]" : "text-sm"}`}>
    ...
    <tbody>{pagination.items.map((line, index) => { const base = unitName(line.base_unit || baseUnit(line.detail_supplier_quotation_purchase_request_detail));
      const cell = supplier ? "px-4 py-3.5 text-slate-600" : "p-3";
      return <tr className={`border-t border-slate-100 ${supplier ? "transition-colors hover:bg-blue-50/30" : ""}`} key={line.detail_supplier_quotation_id || line.detail_purchase_order_id || line.clientId || pagination.offset + index}>
      ...
      </tr>; })}</tbody></table></div><Pagination pagination={pagination} label="Halaman rincian barang" /></div>;
}
export function QuantitySummary({ summary = [], requests = [], variant = "default" }) {
  const supplier = variant === "supplier";
  const pagination = usePagination(summary, summary[0]?.detail_purchase_request_id);
  return <div className="space-y-2">{pagination.items.map(row => { const pr = requests.find(p => p.detail_purchase_request_id === row.detail_purchase_request_id);
    ...
    </div>; })}<Pagination pagination={pagination} label="Halaman kesesuaian kuantitas" /></div>;
}

                    ...
                    <DialogPanel transition className="relative w-full max-w-lg overflow-hidden rounded-xl bg-white text-left shadow-xl outline -outline-offset-1 outline-black/10">
                        ...
                        <form onSubmit={onSubmit}>
                            ...
                            <div className="space-y-5 px-6 pb-6">
                                ...
                                {/* NAMA SUPPLIER */}
                                <div>
                                    <label htmlFor="supplier_name" className="mb-2 block text-sm font-medium leading-5 text-gray-700">
                                        Nama Supplier
                                    </label>

                                    <input id="supplier_name" name="supplier_name" type="text" value={form.supplier_name} onChange={onChange} disabled={loading} autoFocus placeholder="Masukkan nama supplier" className="block h-11 w-full rounded-lg border border-gray-300 bg-white px-3 text-sm text-gray-900 shadow-sm outline-none placeholder:text-gray-400 focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 disabled:bg-gray-100 disabled:text-gray-500" />
                                </div>

                                {/* Phone */}
                                <div>
                                    <label htmlFor="phone" className="mb-2 block text-sm font-medium leading-5 text-gray-700">
                                        Telepon
                                    </label>

                                    <input id="phone" name="phone" type="number" min="0" value={form.phone} onChange={onChange} disabled={loading} className="block h-11 w-full rounded-lg border border-gray-300 bg-white px-3 text-sm text-gray-900 shadow-sm outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 disabled:bg-gray-100 disabled:text-gray-500" />
                                </div>

                                {/* Address */}
                                <div>
                                    <label htmlFor="address" className="mb-2 block text-sm font-medium leading-5 text-gray-700">
                                        Alamat
                                    </label>

                                    <input id="address" name="address" type="text" value={form.address} onChange={onChange} disabled={loading} autoFocus placeholder="Masukkan alamat supplier" className="block h-11 w-full rounded-lg border border-gray-300 bg-white px-3 text-sm text-gray-900 shadow-sm outline-none placeholder:text-gray-400 focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 disabled:bg-gray-100 disabled:text-gray-500" />
                                </div>
                            ...
                            <div className="flex flex-row-reverse items-center gap-3 border-t border-gray-200 bg-gray-50 px-6 py-4">
                                ...
                                <button type="submit" disabled={loading} className="inline-flex w-auto items-center justify-center rounded-lg bg-blue-600 px-4 py-2.5 text-sm font-semibold text-white shadow-sm hover:bg-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50">
                                    {loading ? "Menyimpan..." : editingSupplier ? "Simpan Perubahan" : "Tambah Supplier"}
                                </button>

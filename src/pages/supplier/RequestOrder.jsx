...
function RequestOrder() {
  ...
  const detailRows = selected?.requestSupplierPurchaseRequest?.purchaseRequestDetailPurchaseRequest || [];
  const itemName = (row) => row?.detailPurchaseRequestItem?.item_name || row?.item?.item_name || row?.item_id || "-";
  ...
  const openQuotation = () => {
    ...
    setPrices(p); setDiscounts(d); setPackageDiscount(0); setQuotationNumber(""); setQuotationDate(new Date().toISOString().slice(0, 10)); setValidUntil(""); setNotes(""); setQuotation(selected); setError("");
  };
  ...
  const submitQuotation = async (e) => {
    ...
    try {
      if (!quotationNumber.trim()) throw new Error("Nomor quotation wajib diisi.");
      ...
      await SupplierQuotationService.create(selected.request_supplier_id, {
        quotation_number: quotationNumber.trim(), quotation_date: quotationDate, valid_until: validUntil || null,
  ...
  return <section className="space-y-5">
    <div><p className="text-sm font-medium text-blue-600">Supplier</p><h1 className="mt-1 text-2xl font-bold text-slate-900">Request Order</h1><p className="mt-1 text-sm text-slate-500">Request yang dikirim perusahaan kepada supplier.</p></div>
